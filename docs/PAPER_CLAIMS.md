...
## S04-CLARIFY-02 claim boundary

Permissible paper statement: 20 aligned Attachment4 records have frozen M2 predictions and feature-space explanations; exact tokenizer/continuous-text replay establishes their text positional correspondence under a declared numerical backend envelope. Actual audio/video PTS and forced alignment provide **estimated** acoustic word intervals on 12 records; 8 have text spans only. The 20 original predictions are unchanged. This does not certify official audio/vision feature-to-time production, human word-boundary accuracy or causal explanations. Never describe the Q1 66 mechanical checks as human validated; actual human checks remain 0/100. Attachment3 has **no** prediction CSV because clean encoder reproduction does not settle its ID-100 corrupted-input semantics. Do not report an Attachment3 metric, transfer old M2 metrics to a hypothetical M2-E1, or call the current paper a complete final submission. Verified aggregates, negative outcomes and digest references: `reports/s04_clarify_02/RESULT.json`.
## S04-DEPLOY-03 permissible claims and limits

VERIFIED: under the pinned local general BERT tokenizer, positive token ID100 is `[UNK]` (PAD0/MASK103 and label ignore -100 differ). The single visible-UNK frontend passed an all728 VALID clean interface replay within a prespecified numerical backend envelope; this does not establish that original missing words or the organizer's feature-generation path were recovered. The 32-case preselected UNK stress diagnostic changed 14 predicted classes and is not a model-performance result. After independent review and preinference Git binding, frozen M2/V1 produced 30 unlabeled aligned Attachment3 predictions, readback and source verified; the private CSV SHA256 is `27a62a2a262399a3ecf48e6998fb26703329c0bf2c1f1837ce2f50d225e92adb`. No Attachment3 Accuracy, macro-F1, MAE or Pearson exists. Historical old M2 VALID/attempted96 metrics may be discussed only in their original scope.

Attachment4 remains PARTIAL: original20 predictions/contributions unchanged; all120 key windows have text word-index positions (39 also contain unmapped token positions), 12 records have estimated acoustic intervals, eight only text spans. A word interval plus nearest actual video PTS is not proof of official audio/vision feature-to-time alignment, and 0 records were manually checked. Q1 100 extracted, 66 mechanically checked, 34 failed/unaligned and 0 human verified remain distinct. The new DOCX is a draft for team and page review, not a complete anonymous submission. Source aggregate: `reports/s04_deploy_03/RESULT.json` and `reports/s04_deploy_03/A4_COVERAGE.json`.
